...
model = torchvision.models.resnet18(weights=None) #사전학습된 모델 불러오기
model.conv1 = nn.Conv2d(3, 64, kernel_size=(3,3), stride=(1,1), padding=(1,1), bias=False) #커널 크기 3x3, 스트라이드 1, 패딩 1
model.fc = nn.Linear(512, 10) #출력 클래스 10개로
model = model.to(device) #모델을 GPU로 이동
model.load_state_dict(torch.load('./pretrained_model.pth')) #가중치 불러오기
